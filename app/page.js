  ...
  return (
    <main id="top">
      ...
      <section className="hero container">
        ...
        <div className="hero-actions"><a className="button button-green" href="#contact">{t("Book a personalised demo")} <Icon name="arrow" size={17}/></a><a className="button button-ghost" href="#platform"><span className="play-icon"><Icon name="play" size={13}/></span>{t("Explore the platform")}</a></div>
      ...
      <section className="section container" id="platform"><div className="section-intro"><div><span className="kicker">{t("Built for charging operations")}</span><h2>{t("Turn support moments into confident resolutions.")}</h2></div><p>{t("When a driver is stuck, context matters. EV24 puts the conversation, customer history and charger status together, giving agents and AI the complete picture.")}</p></div>
        <div className="outcome-grid">
          ...
          <article className="outcome-card"><span className="number">02</span><div className="ai-orbit"><div className="orbit orbit-one"/><div className="orbit orbit-two"/><span><Icon name="spark" size={27}/></span><div className="orbit-label label-one">{t("Summarise")}</div><div className="orbit-label label-two">{t("Connect")}</div><div className="orbit-label label-three">{t("Suggest")}</div></div><h3>{t("AI that assists, not interrupts.")}</h3><p>{t("Summarise conversations, surface relevant context and prepare responses while your team stays in control.")}</p></article>
      ...
      <section className="cta-section" id="contact"><div className="container"><div className="cta-panel"><div><span className="kicker kicker-light">{t("Support the next charge")}</span><h2>{t("Give every driver a better answer.")}</h2><p>{t("See how EV24 can connect your conversations, calls and charging operations.")}</p></div><div className="cta-actions"><a className="button button-white" href="mailto:ev24@wattzhub.com">{t("Book a personalised demo")} <Icon name="arrow" size={17}/></a><span>ev24@wattzhub.com<br/>+33 1 85 41 28 67</span></div></div></div></section>

      <footer><div className="container footer-main"><div className="footer-brand"><Brand/><p>{t("Intelligent customer support for electric mobility.")}</p></div><div><strong>{t("Platform")}</strong><a href="#platform">{t("AI assistance")}</a><a href="#channels">{t("Omnichannel inbox")}</a><a href="#network">{t("Integrations")}</a></div><div><strong>{t("Company")}</strong><a href="#about">{t("About EV24")}</a><a href="mailto:ev24@wattzhub.com">{t("Contact")}</a><a href="https://www.linkedin.com/showcase/ev24-by-wattzhub">LinkedIn</a></div><div><strong>{t("Office")}</strong><p>3 Boulevard Thomas Gobert<br/>91120 Palaiseau<br/>Paris-Saclay, France</p></div></div><div className="container footer-bottom"><span>© {new Date().getFullYear()} EV24 by WattzHub</span><span>{t("Built for electric mobility across Europe")}</span></div></footer>
    </main>
  );
}
